...
Uses absolute path to the database.
"""
import sys
import json
import sqlite3
...
# Constants
DB_PATH = os.path.abspath("the_plot_thickens.db")
DEBUG = True
...
def run_server():
    ...
    try:
        ...
        log(f"Connected to database. Table count: {table_count}")
        conn.close()
    except Exception as e:
        log(f"Error connecting to database: {str(e)}")
